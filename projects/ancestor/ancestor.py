...
def earliest_ancestor(ancestors, starting_node):
    ...
    for pair in ancestors:
        g.add_vertex(pair[0]) # adding the vertex
        g.add_vertex(pair[1])
    ...
    oldest_ancestor = -1 # this is what is returned if the node has no parents
    max_path_length = 1

    while q.size() > 0:
        path = q.dequeue()

        vertex = path[-1] # [-1] = the last position in the list
